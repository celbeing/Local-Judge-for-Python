...
- `수업` 메뉴에 `수업 열기...`가 추가되었습니다.
- 수업 ZIP을 열면 Local Judge 앱 데이터 폴더 아래 작업 폴더로 압축 해제합니다.
- 수업 ZIP 압축 해제와 문제 스캔은 UI 스레드 밖에서 실행해 앱 멈춤을 줄이도록 처리되어 있습니다.
- 수업 ZIP 내부의 폴더명은 소제목으로 인식하고, 폴더 안의 문제 JSON을 문항으로 표시합니다.
- 메인 화면 왼쪽 문제 표시 영역 위에 수업 탐색기 트리가 표시됩니다.
- 수업 트리에서 문항을 선택하면 해당 문제 JSON을 현재 문제로 로드합니다.
- 수업으로 연 문항의 제출 기록은 기존 전역 제출 이력이 아니라 수업 작업 폴더 내부에 저장됩니다.
  - 저장 위치: `{lesson-root}\.localjudge\submissions\...`
- 수업 문항 상태 표시:
  - 제출 없음: 기본색
  - 제출 있음, AC 없음: 빨간색과 마지막 판정 suffix
  - AC 있음: 초록색
- 수업 문항에서는 문제 수정과 단일 문제 제출 이력 내보내기를 비활성화합니다.
...
- 수업 작업 폴더 ZIP으로 제출하기
  - 현재 학생은 수업 작업 폴더를 직접 압축해 제출하는 흐름입니다.
  - 앱 내부에서 `수업 결과 ZIP 만들기...`를 제공하면 제출 UX가 좋아집니다.
- KaTeX 오프라인 번들 적용
