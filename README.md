...
문제 설명, 입력 설명, 출력 설명에는 일부 Markdown 문법과 간단한 LaTeX 스타일 수식을 사용할 수 있습니다. 현재 수식 렌더링은 KaTeX/MathJax 전체 문법이 아니라 Local Judge의 자체 간단 렌더러 기준으로 동작합니다.

인라인 수식은 `$...$`로 작성합니다.

```md
정수 $N$이 주어진다. 단, $1 \leq N \leq 100$ 이다.
```

블록 수식은 `$$`를 단독 줄에 두고 그 사이에 작성합니다.
...
지원되는 주요 기호는 다음과 같습니다.

| 종류 | 문법 |
| --- | --- |
| 비교 | `\leq`, `\le`, `\geq`, `\ge`, `\neq`, `\ne` |
| 연산 | `\times`, `\cdot`, `\pm` |
| 특수 기호 | `\infty`, `\ldots`, `\dots`, `\sum`, `\prod`, `\rightarrow`, `\to` |
| 그리스 문자 | `\alpha`, `\beta`, `\gamma`, `\delta`, `\epsilon`, `\theta`, `\lambda`, `\mu`, `\pi`, `\sigma`, `\phi`, `\omega` |
| 분수 | `\frac{a}{b}` |
| 제곱 | `x^2`, `x^{10}` |
| 아래첨자 | `a_i`, `a_{i+1}` |
| 제곱근 | `\sqrt{N}` |
...
- `\frac{\frac{a}{b}}{c}` 같은 중첩 분수는 제대로 지원되지 않을 수 있습니다.
- `\begin{align}`, `matrix`, `cases`, `array` 같은 LaTeX 환경 문법은 지원하지 않습니다.
- `\left`, `\right`, `\int`, `\log`, `\sin` 같은 명령은 현재 별도 처리되지 않습니다.
- 예제 입력/출력 박스 안에서는 수식이 렌더링되지 않고 그대로 텍스트로 표시됩니다.
